...
export function Login() {
    ...
    const handleGoogleLogin = () => {
        setIsLoading(true);
        // Simulação rápida para efeito de UX antes de Lovable Supabase Auth Real
        setTimeout(() => {
            localStorage.setItem('isAuthenticated', 'true');
            // Mock de usuário temporário
            localStorage.setItem('user', JSON.stringify({ name: 'Dra. TM', email: 'contato@teoriadamente.com' }));
            navigate('/pacientes', { replace: true });
        }, 800);
    };
    ...
    return (
        <div className="min-h-screen bg-industrial-bg flex flex-col justify-center items-center p-4 relative overflow-hidden text-industrial-text">
            ...
            <div className="bg-industrial-surface z-10 w-full max-w-md p-8 rounded-2xl shadow-xl border border-industrial-border relative">
                ...
                <div className="space-y-4">
                    <button
                        ...
                        disabled={isLoading}
                        className="w-full flex items-center justify-center gap-3 bg-industrial-surface hover:bg-gray-50 dark:hover:bg-gray-800 text-industrial-text font-medium py-3 px-4 rounded-xl border border-industrial-border transition-all shadow-sm active:scale-[0.98]"
                    >
                        {isLoading ? (
            ...
            <div className="mt-8 text-industrial-text-muted text-xs">Versão 1.0.0 • Infinite Photon System</div>
        </div>
    );
}
