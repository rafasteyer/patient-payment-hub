...
export function Goals() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      await financeService.updateGoal(newGoal);
      const updatedGoals = await financeService.getGoals();
      ...
      <div className="bg-industrial-surface p-8 rounded-xl shadow-sm border border-industrial-border max-w-2xl">
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="pt-4 flex items-center justify-between">
            <span className="text-green-600 text-sm font-medium h-5 block">{message}</span>
            <button
              type="submit"
